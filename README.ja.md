...
>
> **追記:** `uv venv --python 3.11` で明示的にバージョンを指定してからインストールするのが一番トラブルが少なかったです。`uv venv --python 3.11 && source .venv/bin/activate && uv pip install -e .` の順番で実行するとスムーズです。

## クイックスタート

### macOSアプリ

ApplicationsフォルダからoMLXを起動します。ウェルカム画面が3つのステップを案内します — モデルディレクトリの設定、サーバー起動、最初のモデルダ